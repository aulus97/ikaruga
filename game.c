...
int main(int argc, char **argv){
	...
	void altera_color(int nave_color){
		...
		*/
		struct coordenada_enemygreen en_gr_coord[6];
			int i;
			for(i = 0; i<6;i++){
		...
		struct coordenada_enemyblue en_bl_coord[6];
			for(i = 0; i<6;i++){
			...
			//posicoes iniciais en_bl:
				...
				en_bl_coord[i].en_bl_dy = 0.51;//<<<<<----- COM 0.01 DE DIFERENÇA JA MUDA!
			}
	   ...
	   bool redraw = true;
			...
			//outras possibilidades?: (*p[i]).jfje || (*p)[i].hrhre ||(*p+i).jhjdf  <MAS CUIDADO PARA NAO SE LIMITAR!!!
			void move_en_gr(struct coordenada_enemygreen *p, float nave_x){										
					//descendente
			...
			void teletrans_en_gr(struct coordenada_enemygreen *g, int j){
			...
			void collision_en_gr(struct coordenada_enemygreen eg, int *en_gr_alive, struct coordenada_nave n, int *score, int *nave_alive){									
			...
			void move_en_bl(struct coordenada_enemyblue *c, float nave_x){									
					//descendente
			...
			void teletrans_en_bl(struct coordenada_enemyblue *b, int j){
				
				srand((unsigned)b->en_bl_x);
				float k = j + (1 + rand()%8); //RESOLVA ISSO:::: 9 + 4 = 13!!! -->>>> ISSO EH FORA DA TELA!!!!							
					if(k > 9){
						if((int)k%2 == 0){k /= 2;}
						else{k = ( (k - 1)/2 ) - 1;}											
					}
			...
			void collision_en_bl(struct coordenada_enemyblue eb, int *en_bl_alive, struct coordenada_nave n, int *score, int *nave_alive){									
					//se alguma destas condicoes ocorrerem, entao NAO ha colisao, caso contrario HA colisao
	   ...
	   if(!display) {
		...
		//cria os bitmaps quadrangulares MANUALMENTE de tamanho EN_BL_SIZE (variavel global declarada acima)
			...
			en_bl[5] = al_create_bitmap(EN_BL_SIZE, EN_BL_SIZE);
	   ...
	   while(playing)
	   {
		  ...
		  al_wait_for_event(event_queue, &ev);
				...
				if(ev.type == ALLEGRO_EVENT_TIMER) {
					...
					for(i= 0; i<6; i++){
						//teletransporte do en_gr?
							teletrans_en_gr( &(en_gr_coord[i]), i);
						//analise de colisao do en_gr 
							collision_en_gr( en_gr_coord[i], &(en_gr_coord[i].en_gr_alive),nave_coord,&score,&nave_coord.nave_alive);
						if(en_gr_coord[i].en_gr_alive == 1){	
							//movimento de en_gr
							move_en_gr(&(en_gr_coord[i]), nave_coord.nave_x);
						}else{teletrans_en_gr( &(en_gr_coord[i]), i);}
					}
					for(i= 5; i>=0; i--){
						//teletransporte do en_bl?
							teletrans_en_bl( &(en_bl_coord[i]), i);
						//analise de colisao do en_bl 
							collision_en_bl( en_bl_coord[i], &(en_bl_coord[i].en_bl_alive),nave_coord,&score,&nave_coord.nave_alive);
						if(en_bl_coord[i].en_bl_alive == 1){
							//movimento de en_bl
							move_en_bl(&(en_bl_coord[i]), nave_coord.nave_x);
						}else{teletrans_en_bl( &(en_bl_coord[i]), i);}
					}
			...
			fwrite(&record,sizeof(int),1, points);*/
				if(record < score){//mude a condiçao para maior que '>' e retorne o recorde para 0
					...
					fclose(points);
						...
						//colore toda a tela de azul muito claro
			...
			//al_draw_text(size_32, al_map_rgb(15, 200, 30), SCREEN_W/3, SCREEN_H/2+100, 0, pontuacao);	
